...
namespace Config {
	...
	// === TIMING ===
	constexpr long long MAX_TIME_SECONDS = 40 * 60;
...
constexpr double RING_MIGRATION_RATE = 0.05;  // 5% of population migrates (was 10%)
...
constexpr double EXPLORATION_VND_EXTRA_PROB = 0.25;  // extra VND chance for promising
...
constexpr double RUIN_INTENSITY_SCALE_EXPLOITATION = 0.15; // exploitation scale → 10-25%
...
constexpr double EDGE_PENALTY_LAMBDA = 0.4;  // increased from 0.2 for stronger diversification
constexpr double EDGE_PENALTY_DECAY = 0.995;
constexpr int GLS_STAGNATION_TRIGGER = 20;
constexpr double GLS_CATASTROPHE_RESET = 0.25;  // keep 25% of penalties after catastrophe
constexpr double GLS_PENALTY_FACTOR = 0.15;     // penalty = (best_fit / num_vehicles) * factor

// === MIGRATION DIVERSITY ===
constexpr int MIGRATION_ELITE_COUNT = 2;        // fixed 2 elite migrants
constexpr int MIGRATION_DIVERSE_COUNT = 3;      // 3 most diverse (max BPD from global best)

// === DYNAMIC IMMUNITY ===
constexpr double PROGRESS_IMMUNITY_SECONDS = 10.0;  // if found NEW BEST in last Xs, block migration

// === DIVERSITY-PULSE MIGRATION === (DISABLED - conflicts with Ring)
constexpr double DIVERSITY_PULSE_INTERVAL_SECONDS = 9999.0;  // effectively disabled
constexpr double DIVERSITY_PULSE_RATE = 0.15;              // 15% of population
constexpr double DIVERSITY_PULSE_BPD_THRESHOLD = 0.20;     // must have >20% BPD difference
...
constexpr double VND_TIGHT_ROUTE_THRESHOLD = 0.95; // routes > 95% capacity are "tight"
